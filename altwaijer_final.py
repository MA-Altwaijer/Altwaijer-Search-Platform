...
# 1. إعدادات المنصة الكلاسيكية المعتمدة
...
if search_query:
    ...
    with col_arabic:
        ...
        st.markdown(f'<a href="https://shamaa.org/" target="_blank"><button style="width:100%; background-color:#17a2b8; color:white; border:none; border-radius:10px; height:3.5em; cursor:pointer; margin-bottom:10px; font-weight:bold;">💎 قاعدة شمعة</button></a>', unsafe_allow_html=True)
    ...
    # 3. الأدوات (تبويبات منظمة) - تم إصلاح SyntaxError هنا
    st.markdown("---")
    ...
    with t2:
        ...
        with c3:
            t_ref = st.text_input("عنوان البحث:")
            l_ref = st.text_input("رابط البحث:")
            if st.button("أرشفة"):
                if t_ref and l_ref:
        ...
        with c4:
            if st.session_state.library:
                ...
                st.write("المكتبة خالية حالياً.")

st.markdown("<p style='text-align: center; color: gray;'>M.A. Altwaijer 2026 - الإصدار الكلاسيكي المستقر</p>", unsafe_allow_html=True)
